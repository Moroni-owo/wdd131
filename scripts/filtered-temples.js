...
document.querySelectorAll("nav a").forEach(link => {
  link.addEventListener("click", e => {
    ...
    if (filter === "old") {
      result = temples.filter(t => parseInt(t.dedicated.slice(0, 4)) < 1900);
    } else if (filter === "new") {
      result = temples.filter(t => parseInt(t.dedicated.slice(0, 4)) > 2000);
    } else if (filter === "large") {
      result = temples.filter(t => t.area > 90000);
    } else if (filter === "small") {
      result = temples.filter(t => t.area < 10000);
    } else if (filter === "nonutah") {
      result = temples.filter(t => !t.location.includes("Utah"));
    }
